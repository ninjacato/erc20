...
use web3::types::{Index, Transaction, H160, H256, U256, U64};
...
impl From<Transaction> for ParsedTransaction {
    ...
    fn from(transaction: Transaction) -> Self {
        match transaction.to {
            None => {
                if transaction.input.0.is_empty() {
        ...
        match parsed_transaction {
            ...
            ParsedTransaction::ContractInvocation(transaction) => {
                ...
                match contract_invocation {
                    TransactionContractInvocation::ERC20(method, transaction) => match method {
                    ...
                    TransactionContractInvocation::Other(_) => {
                        Err(ERC20Error::NoTransferTransaction)
                    }
                }
            }
    ...
    pub fn get_from_to_value(&self) -> Result<(H160, H160, U256), ERC20Error> {
        ...
        match self.transfer_type {
            TransferType::Ethereum => {
                if let Some(v) = self.transaction.from {
                    from_v = v;
                } else {
                ...
                match contract_invocation {
                    TransactionContractInvocation::ERC20(method, transaction) => match method {
                        ERC20Method::Transfer => {
                            let mut resp: BytesToFixedNumber = transaction.input.clone().into();
                            let _ignore = resp.next_vec(4);
                            if let Some(v) = transaction.from {
                                from_v = v;
                            } else {
                                return Err(ERC20Error::NoTransferTransaction);
                            }
                        ...
                        ERC20Method::TransferFrom => {
                            let mut resp: BytesToFixedNumber = transaction.input.into();
                            let _ignore = resp.next_vec(4);
...
impl Transfer for TransactionAndTransferType {
    fn from(&self) -> H160 {
        match self.get_from_to_value() {
            ...
            Err(err) => {
                panic!(
                    "Unexpected transaction for TransactionAndTransferType::from {:?}",
                    err
                )
            }
        }
    ...
    fn to(&self) -> H160 {
        match self.get_from_to_value() {
            ...
            Err(err) => {
                panic!(
                    "Unexpected transaction for TransactionAndTransferType::to {:?}",
                    err
                )
            }
        }
    ...
    fn contract(&self) -> Option<H160> {
        match self.transfer_type {
            ...
            TransferType::ERC20 => match self.transaction.to {
    ...
    fn value(&self) -> U256 {
        match self.get_from_to_value() {
            ...
            Err(err) => {
                panic!(
                    "Unexpected transaction for TransactionAndTransferType::value {:?}",
                    err
                )
            }
        }
